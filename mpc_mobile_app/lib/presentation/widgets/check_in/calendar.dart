...
import 'package:mpc_mobile_app/presentation/widgets/check_in/streak.dart';
import 'package:mpc_mobile_app/presentation/widgets/check_in/workouts.dart';
      ...
      children: [
        if (isExpanded)
          CalendarGrid(
            ...
            daysThatHaveCheckIns: [...widget.daysThatHaveCheckIns],
          )
        else if (!isExpanded)
          DaysOfTheWeek(),
        ...
        Row(
          children: [
            CheckInStreak(user: widget.user),
            Gap(10.w),
            WorkoutsAmount(
              user: widget.user,
              selectedMonth: isExpanded ? widget.selectedMonth : null,
              selectedYear: isExpanded ? widget.selectedYear : null,
            ),
          ],
        ),
      ],
...
CalendarData buildCalendarGrid({required int month, required int year}) {
...
class CalendarGrid extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final calendarGrid = buildCalendarGrid(month: month, year: year);
    final monthNames = [
      'January',
      'February',
      'March',
  ...
  List<Color> getColors(List<int> daysThatHaveCheckIns) {
    if (isCurrentMonth && daysThatHaveCheckIns.contains(day)) {
      return [AppColors.errorColor.withValues(alpha: 0.8), Colors.transparent];
        ...
        month == DateTime.now().month &&
        year == DateTime.now().year) {
      return [Colors.transparent, AppColors.errorColor.withValues(alpha: 0.8)];
    } else if (isCurrentMonth) {
      return [
