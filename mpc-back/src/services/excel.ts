...
import { Exercise, TrainingDay, TrainingPlan } from '../models/User';
...
export class TrainingPlanExcelService {
  /**
  ...
  async generateFromTemplate(
    ...
    trainingPlan.days.forEach((day, dayIndex) => {
      if (dayIndex >= dayConfig.length) return;
      ...
      day.exercises.forEach((exercise, exerciseIndex) => {
        if (exerciseIndex >= config.maxExercises) return;
   ...
   */
  async generateBufferFromTemplate(
    trainingPlan: TrainingPlan,
    options: Omit<ExcelGeneratorOptions, 'outputPath'>
  ): Promise<Buffer> {
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile(options.templatePath);
  ...
  async generateNew(
    ...
    trainingPlan.days.forEach((day, dayIndex) => {
      ...
      day.exercises.forEach((exercise, exerciseIndex) => {
        ...
        const weightStr = this.formatWeights(
   ...
   */
  private formatWeights(weights: number[]): string {
   ...
   */
  private formatRir(rirs: number[]): string {
   ...
   */
  async parseFromExcel(filePath: string): Promise<TrainingPlan> {
    const workbook = new ExcelJS.Workbook();
    ...
    dayMarkers.forEach((headerRow, dayIndex) => {
      const day: TrainingDay = {
        name: String(
          sheet.getCell(`B${headerRow}`).value || `DAY ${dayIndex + 1}`
        ),
        exercises: [],
      };
