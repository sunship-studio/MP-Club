...
authRouter.post('/new-password', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const dbToken = yield PasswordResetToken_1.default.findOne({
        token: tokenHash,
        used: false,
    });
    if (!dbToken) {
    ...
    const user = yield User_1.default.findById(dbToken.userId);
    if (!user) {
    ...
    if (user) {
        const result = yield auth_1.AuthController.setPassword(user.email, password);
        res.setHeader('authorization', (result === null || result === void 0 ? void 0 : result.token) || '');
        res.setHeader('x-refresh-token', (result === null || result === void 0 ? void 0 : result.refreshToken) || '');
        res.json(user);
    }
...
authRouter.get('/verify-reset-token/:token', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { token } = req.params;
        const hashedToken = crypto_1.default.createHash('sha256').update(token).digest('hex');
        const resetTokenDoc = yield PasswordResetToken_1.default.findOne({
            token: hashedToken,
            used: false,
        });
...
authRouter.post('/forgot-password', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        try {
            yield sendPasswordResetEmail(email, deepLink);
        }
        catch (emailError) {
        ...
        // Send email
        res.json(Object.assign({ message: 'Password reset link sent to your email' }, (process.env.NODE_ENV === 'development' && { resetLink: deepLink })));
    }
    catch (error) {
