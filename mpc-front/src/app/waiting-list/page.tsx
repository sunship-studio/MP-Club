      ...
      return (
        <div className="flex flex-col items-center justify-center py-8 px-6">
          <h1 className={`text-2xl md:text-3xl font-semibold mb-4 text-center`}>
            Waiting List for 1-on-1 Coaching
          </h1>
          ...
          <h2 className="text-lg font-semibold text-left w-full md:w-2/3">
            I am available on:{" "}
          </h2>
          {/* Desktop Available */}
          <div className="hidden md:flex flex-row w-2/3 space-x-2">
            <AvailableBox
              weekday={availableWeekdays[0]["name"]}
              value={availableWeekdays[0]["value"]}
              setValue={updateWeekdayValue}
            />
            <AvailableBox
              weekday={availableWeekdays[1]["name"]}
              value={availableWeekdays[1]["value"]}
              setValue={updateWeekdayValue}
            />
            <AvailableBox
              weekday={availableWeekdays[2]["name"]}
              value={availableWeekdays[2]["value"]}
              setValue={updateWeekdayValue}
            />
            <AvailableBox
              weekday={availableWeekdays[3]["name"]}
              value={availableWeekdays[3]["value"]}
              setValue={updateWeekdayValue}
            />
            <AvailableBox
              weekday={availableWeekdays[4]["name"]}
              value={availableWeekdays[4]["value"]}
              setValue={updateWeekdayValue}
            />
            <AvailableBox
              weekday={availableWeekdays[5]["name"]}
              value={availableWeekdays[5]["value"]}
              setValue={updateWeekdayValue}
            />
          </div>
          {/* Mobile Available */}
          ...
          <div className="flex flex-col items-start w-full md:w-2/3 mb-4">
            {availableWeekdays.map(
              (day, index) =>
                day.value && (
                  <div
                    className="flex flex-col w-full md:w-2/3 space-y-2 py-2"
                    key={day.name}
                  >
                    ...
                    <ErrorText
                      error={weekdaysErrors[`weekdays.${index}.startTime`]}
                    />
                    <ErrorText
                      error={weekdaysErrors[`weekdays.${index}.endTime`]}
                    />
                  </div>
                )
            )}
